...
std::string CurrentAccount::getDisplay() const {
    ...
    out2 << std::fixed << std::setprecision(2) << MinimumBalance;
    return "Account Number: " + accountNo + "\nAccount Holder: " + holderName + "\nBalance: PKR " +  out1.str() + "\nMinimum Balance Requirment: PKR " + out2.str() + "\n";
}
...
void DataBase::addAccount(const std::string& name,const std::string& passcode,const double amount,const bool status) {
